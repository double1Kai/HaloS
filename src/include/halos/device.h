#ifndef HALOS_DEVICE_H
#define HALOS_DEVICE_H
...
enum device_type_t{
    DEV_NULL,   //空设备
    DEV_CHAR,   //字符设备，以char为单位读写，显示器、键盘
    DEV_BLOCK,  //块设备，以块为单位读写，硬盘
};

//设备子类型
enum device_subtype_t{
    DEV_CONSOLE = 1,//控制台，得和空分开
    DEV_KEYBOARD,   //键盘
    DEV_IDE_DISK,   //IDE磁盘
...
//设备控制命令
enum device_cmd_t{
    DEV_CMD_SECTOR_START = 1,   //获取设备扇区开始位置
...
typedef struct request_t
{
    ...
    list_node_t node;      //列表节点，将请求放到链表中
} request_t;
...
typedef struct device_t
{
    ...
    dev_t dev;          //设备号
    dev_t parent;       //父设备号
    void *ptr;          //设备指针，唯一设备用上，比如显示器和键盘，磁盘的话用得上
    list_t request_list;//块设备需要的请求链表  
    bool direct;        //磁盘寻道方向

    //设备控制的函数指针，如果参数不匹配，多的参数会被过滤掉
    int (*ioctl)(void *dev, int cmd, void *args, int flags);
    //读设备的函数指针
    int (*read)(void *dev, void *buf, size_t count, idx_t idx, int flags);
    //x写设备的函数指针
    int (*wrtie)(void *dev, void *buf, size_t count, idx_t idx, int flags);
} device_t;
