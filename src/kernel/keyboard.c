...
static char keymap[][4] = {
    ...
    //两个bool预防左右shift win alt这种，用于检测按键是否被按下
    /* ---------------------------------- */
...
static bool extcode_state;//扩展码状态，不如alt ctrl fn这些
...
static void keyboard_ack(){
    ...
    }while (state != KEYBOARD_CMD_ACK);//等待键盘发送ack
}
...
void keyboard_handler(int vector){
    ...
    u16 scancode = inb(KEYBOARD_DATA_PORT);//读取按键信息扫描码
    u8 ext = 2;//当前的按键keymap状态索引，默认没有shift键，即第二个，也可能是3
    ...
    //获取通码
    u16 makecode = (scancode & 0x7f);//取后七位
    ...
    else if(numlock_state && (0x47 <= makecode && makecode <= 0x53)){
        ch = INV;
    }
    else{
...
u32 keyboard_read(char *buf, u32 count){
    ...
    while (nr < count)
    {
        ...
        {
            waier = running_task();
            task_block(waier, NULL, TASK_BLOCKED);
        }
